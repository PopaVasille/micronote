...
// Local state for the shopping list
const localShoppingItems = ref([]);
const hasUnsavedChanges = ref(false);

// We use Inertia's useForm to manage the state of the edit form
const form = useForm({
    id: null,
    title: '',
    content: '',
...
// When the `note` prop changes, we update the form and the shopping items
watch(() => props.note, (newNote) => {
    if (newNote) {
        ...
        // Initialize the local shopping list with a deep copy
...
// Computed property for the formatted note type
...
// Computed property for the note type badge color
...
// Function to toggle shopping items (local only)
...
console.log('Save shopping list:', hasUnsavedChanges.value);
    // Send only the updated list to the server
    form.transform(data => ({
        ...data,
        metadata: {
            ...props.note.metadata,
        ...
        onSuccess: (page) => {
            ...
            // Emit an event to update the list in the dashboard
...
const cancelEditing = () => {
    // Reset the form to the original values of the note
    form.reset();
    isEditing.value = false;
};
...
// Save function for text content
const saveChanges = () => {
    if (!props.note) return;
    ...
    form.post(route('notes.update', props.note.id), {
        ...
        onSuccess: (page) => {
            isEditing.value = false;
            emit('noteUpdated', page.props.updatedNote || props.note);
            emit('close');
        },
        onError: (errors) => {
...
// Computed property for the shopping list progress
